    ...
    <div className="space-y-4">
      ...
      {connected && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-white p-3 rounded-lg shadow text-center ring-2 ring-red-100">
            <div className="text-sm text-gray-500">심박수 (HR)</div>
      ...
      {connected && !data && (
        <p className="text-sm text-amber-900 bg-amber-50 border border-amber-200 rounded-md p-3 leading-relaxed">
          위 수치는 <strong>엣지 → Kafka</strong>로 올 때 갱신됩니다. <strong>심박(HR)</strong>은{' '}
          <code className="text-xs bg-white px-1 py-0.5 rounded">edge_devices.user_id</code>·엣지 실행을 확인하세요
          (WBGT만 먼저 잡힐 수 있음).
        </p>
      )}
